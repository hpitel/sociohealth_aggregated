...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["data.csv",new URL("https://raw.githubusercontent.com/hpitel/sociohealth_aggregated/main/health_index_all%20states3.csv",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Reported Health Indicators by State (Average by County)`
)});
  main.variable(observer("viewof order")).define("viewof order", ["html"], function(html)
{
  const options = [
    ...
    {label: "Health Index, ascending", value: (a, b) => a.value - b.value},
    {label: "Health Index, descending", value: (a, b) => b.value - a.value}
  ];
...
d3.csvParse(await FileAttachment("data.csv").text(), ({state, poor_health}) => ({name: state, value: +poor_health}))
)});
